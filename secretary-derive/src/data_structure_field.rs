...
use crate::{field_types::{detect_task_field_type, TaskFieldType}, utilities::{convert_to_json_type, get_instruction}};
...
impl DataStructureField {
    pub fn new(field: Field, name: String, instruction: String, json_data_type: String, task_field_type: TaskFieldType) -> Self {
        Self { field, name, instruction, json_data_type, task_field_type }
    }
...
pub fn get_data_structure_fields(data: &Data) -> Result<Vec<DataStructureField>, TokenStream> {
    match data {
        Data::Struct(content) => {
            let named_fields: syn::punctuated::Punctuated<Field, syn::token::Comma> = match &content.fields {
        ...
        Data::Enum(enum_data) => {
            let error = syn::Error::new_spanned(&enum_data.enum_token, "Enums are not supported yet");
            Err(TokenStream::from(error.to_compile_error()))
        },
        Data::Union(union_data) => {
            let error = syn::Error::new_spanned(&union_data.union_token, "Unions are not supported");
            Err(TokenStream::from(error.to_compile_error()))
        }
    }
}
